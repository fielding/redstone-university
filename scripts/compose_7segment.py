...
names = ['7seg-wall', '7seg-lit', '7seg-closeup']
...
for n in names:
    ...
    panels.append(p)
H = max(p.height for p in panels[:2])
scaled = []
for i, p in enumerate(panels):
    h = H if i < 2 else int(H * 0.5)
    scaled.append(p.resize((round(p.width * h / p.height), h), Image.LANCZOS))
GAP = 90
PAD = 48
W = sum(p.width for p in scaled) + GAP * (len(scaled) - 1) + PAD * 2
out = Image.new('RGBA', (W, H + PAD * 2), (0, 0, 0, 0))
x = PAD
for p in scaled:
    out.paste(p, (x, PAD + H - p.height), p)
    x += p.width + GAP
