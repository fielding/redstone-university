...
> assets (testbench, mattbatwings references, canonical builds), the build language,
...
> findings + the mattbatwings world import.
...
| **The Realizations** | (a) testbench machine, (b) mattbat references, (c) canonical book builds | Prove it works / reference designs / what readers actually build | (a) proven, (b) exported, (c) **not started — the main build work ahead** |
...
| Module | Artifact (course) | Spec section | Testbench proof | mattbat reference | Notes |
| :-- | :-- | :-- | :-- | :-- | :-- |
...
| 3b Interlude | compact design | — | — | mattbat center-logic is a compact exemplar | Compact = aside, not vehicle |
...
| 13 Double-dabble | BCD converter | — | — | mattbat uses BCD display logic | |
...
4. **Canonical = hand-buildable, modular, legible** (the mattbatwings / Ben Eater
...
| mattbat world | `server/world-mattbat-8bit` (**currently active**; source in `~/Downloads`) | mattbatwings' 8-bit computer (1.19→1.21.6), Ben-Eater-style, at spawn |
| `mattbat-8bit-full.schem` | `assets/schematics/mattbat/` (in-repo; also on the server) | Whole computer, 281,853 blocks |
| `mattbat-red-left.schem` | 〃 | Register-file section (~33k blocks) |
| `mattbat-purple-right.schem` | 〃 | RAM / control-ROM grid (~29k blocks) |
| `mattbat-center-logic.schem` | 〃 | ALU + flags + control core (~27k blocks) |
| `mattbat-ref-*-bitslice.schem` ×3 | 〃 | **Single-unit references** — one bit-slice each of register file, RAM bit-plane, ALU core. His machine is 8 identical bit-slices stacked vertically (period-2 layers, 95–98% autocorrelation), so one slice fully characterizes each design. See the folder README for provenance/credit and course-use notes. |
...
mattbat sections are **reference implementations** (8-bit, compact-leaning) — for
...
testbench code (verified geometry)
        ...
        ▼
canonical module build (plain, wire-role colors)     ←— mattbat refs consulted
...
3. ✅ DONE (2026-07-06) — **Single-component references carved** from the mattbat
