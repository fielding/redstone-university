...
![RAM Architecture Diagram](./images/ram-architecture-circuitverse.png)
*Figure: A RAM module as an array of registers. The address decoder chooses one location, the inverter bank converts the select lines to active-high, write-enable controls storage, and the gated read path places only one row onto the shared output bus.*
...
![16x4 RAM Minecraft Build](./images/ram-minecraft.png)
*Figure: A 16x4-bit RAM build. The decoder and inverter bank create active-high row selects, the write gates control storage, and the gated read bus presents the selected row on the output.*
